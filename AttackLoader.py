...
class AttackLoader:
    def __init__(self, args, logger, data_type="standard"):
        ...
        if data_type == "standard":
            ...
            self.cache_dir = os.path.join(
                self.cache_dir, args.dataset, args.model_type, args.attack_type
            )
    ...
    def split_csv_to_testval(self):
        ...
        if self.args.dataset == "sst2":
            ...
            csv_files = glob.glob(
                os.path.join(
                    val_path, f"{self.args.model_type}*{self.args.attack_type}.csv"
                )
            )
            assert (
                len(csv_files) == 1
            ), f"{len(csv_files)} exists in validation path {csv_files}"
    ...
    def get_attack_from_csv(self, dtype="test", batch_size=64, model_wrapper=None):
        def clean_text(t):
            t = t.replace("[[", "")
            t = t.replace("]]", "")
            return t
        ...
        successful_adv_idx = ~wrong_pred_idx & (
            df.original_output != df.perturbed_output
        )
      ...
      """
            ...
            )  # ratio to attain max_adv_num number of adv. samples
            ...
            other_split_idx = rand_idx[
                split_point : split_point + num_adv_samples
            ]  # Find equal number of clean samples
        ...
        if (
            "nli" in self.csv_file
        ):  # For NLI dataset, only get the hypothesis, which is attacked
    ...
    def get_attack_from_pkl(self, filename, model_wrapper, batch_size):
        ...
        if ori_len > reduced_len:
            self.logger.log.debug(
                f"{ori_len - reduced_len} samples removed while attacking."
            )
    ...
    """
        ...
        incorrect_indices = np.not_equal(
            df["original_output"].values, df["ground_truth_output"].values
        )
        self.logger.log.info(f"# of adv. predictions different : {adv_pred_diff}")
        self.logger.log.info(f"# of clean predictions different : {clean_pred_diff}")
        self.logger.log.info(f"Clean Accuracy {correct / total}")
