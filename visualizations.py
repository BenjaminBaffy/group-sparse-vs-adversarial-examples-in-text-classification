...
##% Figure
robust_spectrum = pd.read_csv(
    "fig/data/spectrum_kPCA.csv", names=["max", "min"]
).astype(float)
naive_spectrum = pd.read_csv(
    "fig/data/spectrum_naive.csv", names=["max", "min"]
).astype(float)

fig, ax = plt.subplots()
p = ax.scatter(
    robust_spectrum.iloc[:, 1],
    robust_spectrum.iloc[:, 0],
    label="kPCA",
    s=50,
    alpha=0.5,
)
ax.scatter(
    naive_spectrum.iloc[:, 1], naive_spectrum.iloc[:, 0], label="Raw", s=50, alpha=0.5
)
...
(p1,) = ax.plot(
    steps, df["mu"], color=color1, label=r"$\mu$ Relative Error", linewidth=4
)
(p2,) = ax2.plot(
    steps, df["cov"], color=color2, label=r"$\Sigma$ Relative Error", linewidth=4
)
last_value = df["mu"].iloc[-1]
last_step = steps.iloc[-1]
ann = ax.annotate(
    f"{last_value:.2E}",
    xy=(last_step, last_value),
    xytext=(2 ** (12.2), last_value + 0.05),
    fontsize=27,
...
ax2.set_ylabel(
    r"$\frac{||\Sigma-\tilde\Sigma||}{||\Sigma||}$", rotation=90, fontsize=35
)
...
def confidence_ellipse(
    ax, x=None, y=None, precomputed_stats=None, n_std=3.0, facecolor="none", **kwargs
):
    """
...
for cls_idx in np.unique(all_labels):
    ...
    for n_std in range(1, 4):
        confidence_ellipse(
            ax,
            feats[all_labels == cls_idx, 0],
            feats[all_labels == cls_idx, 1],
            n_std=n_std,
            linestyle="--",
            edgecolor="tab:blue",
            label="MLE",
...
steps = {
    "MCD": [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, "MLE"],
    "PCA": [50, 100, 150, 200, 250, 300, "None"],
}
...
##%
paths = [
    "fig/data/fae-bert-imdb.csv",
    "fig/data/fae-roberta-imdb.csv",
    "fig/data/fae-bert-agnews.csv",
    "fig/data/fae-roberta-agnews.csv",
    "fig/data/fae-bert-sst2.csv",
    "fig/data/fae-roberta-sst2.csv",
]
