...
parser.add_argument(
    "--preprocess", default="standard", type=str, choices=["standard", "fgws"]
)
parser.add_argument(
    "--data_type", default="standard", type=str, choices=["standard", "fgws"]
)
parser.add_argument(
    "--target_model",
    default="textattack/bert-base-uncased-imdb",
    type=str,  # textattack/roberta-base-SST-2
    help="name of model (textattack pretrained model, path to ckpt)",
)
parser.add_argument(
    "--model_type", type=str, help="model type (e.g. bert, roberta, cnn)"
)
parser.add_argument(
    "--use_state_dict",
    action="store_true",
    default=False,
    help="whether to load model from a torch state dict",
)
parser.add_argument(
    "--data_root_dir", default="attack-log/original", help="root directory of data"
)

parser.add_argument(
    "--scenario",
    type=str,
    help="scenario that determines how the configure the adv. dataset",
)
parser.add_argument(
    "--include_fae",
    default=False,
    action="store_true",
    help="Include failed adversarial examples for detection as well",
)
...
parser.add_argument(
    "--attack_type", default="textfooler", type=str, help="attack type for logging"
)
parser.add_argument("--exp_name", default="tmp", type=str, help="Name for logging")
...
parser.add_argument(
    "--model_params_path",
    type=str,
    default="params/attention_key-exclude.json",
...
if args.exp_name:
    args.log_path = (
        f"runs/{args.dataset}/{args.exp_name}/{model_type}/{args.attack_type}"
    )
else:
...
if __name__ == "__main__":
    ...
    s_time = time.time()
    feats = detection.get_train_features(
        model_wrapper,
        args,
        batch_size=64,
        dataset=trainset,
    ...
    feats = feats.numpy()
    s_time = time.time()
    reduced_feat, labels, reducer, scaler = misc.preprocess_features(
        feats, params, args, logger
    )

    train_stats, estimators = detection.get_stats(
        reduced_feat,
        labels,
        cov_estim_name=args.cov_estimator,
        use_shared_cov=params["shared_cov"],
        params=params,
    )
    naive_train_stats, naive_estimators = detection.get_stats(
        reduced_feat, labels, cov_estim_name="None", use_shared_cov=params["shared_cov"]
    )
        ...
        for name, stat in zip(["naive", "robust"], all_train_stats):
            for idx, (mu, cov) in enumerate(stat):
                ...
                path_to_csv = os.path.join(
                    os.path.dirname(args.log_path), "spectrum.csv"
                )
                ...
                plt.matshow(cov)
                plt.title(
                    f"Cond.:{kappa:.3e} Max:{max(spectrum):.3e} Min: {min(spectrum):.3e}"
                )
                cb = plt.colorbar()
                cb.ax.tick_params(labelsize=14)
                plt.savefig(
                    os.path.join(os.path.dirname(args.log_path), f"{name}-cls{idx}.png")
                )
        exit()
    ...
    for s in range(args.start_seed, args.end_seed + 1):
        ...
        detector = Detector(
            ...
            logger,
            params,
            (scaler, reducer, all_estimators, args.cov_estimator),
        ...
        if args.baseline:
            ...
            roc, auc, tpr_at_fpr, conf, testset = detector.test(
                args.fpr_threshold, args.pkl_test_path
            )
