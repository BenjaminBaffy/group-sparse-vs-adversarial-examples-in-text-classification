...
class Detector:
    ...
    def get_data(self, pkl_path=None):
        if pkl_path and pkl_path.endswith(".pkl"):
            dataset = self.loader.get_attack_from_pkl(
                pkl_path, self.model_wrapper, batch_size=64
            )
        else:
            dataset, _ = self.loader.get_attack_from_csv(
                batch_size=64, dtype=self.dtype, model_wrapper=None
            )

        adv_count = dataset.result_type.value_counts()[1]
        total = len(dataset)
        self.logger.log.info(
            f"Percentage of adv. samples :{adv_count}/{total} = {adv_count/total:3f}"
        )
        return dataset
    ...
    def test(self, fpr_thres, pkl_path=None):
        ...
        for name, stats, estim in zip(
            ["MLE", self.estimator_name], self.stats, self.estimators
        ):
            ...
            self.logger.save_custom_metric(
                f"{name}-estim.", [tpr_at_fpr, fpr_thres, f1, auc], metric_header
            )
