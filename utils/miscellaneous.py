...
def return_cov_estimator(name, params):
    if name == "OAS":
        ...
        return MinCovDet(
            support_fraction=params["h"] if params.get("h", None) else None
        )
...
def preprocess_features(feats, params, args, logger):
    ...
    if params["sample"]:
        ...
        sample_idx = np.random.choice(
            range(len(feats)), size=num_sample[args.dataset], replace=False
        )
    ...
    if params["reduce_dim"]["do"]:
        if params["reduce_dim"]["method"] == "PCA":
            reduced_feat, reducer = return_PCA_features(
                sampled_feats, params, scaler, logger, seed=0
            )
        elif params["reduce_dim"]["method"] == "RF":
            scaler = None
            reducer = RBFSampler(
                gamma=1, n_components=params["reduce_dim"]["dim"], random_state=0
            )
...
def return_PCA_features(feats, params, scaler, logger, seed=0):
    if (
        params["reduce_dim"]["dim"] < 1
    ):  # Determine number of components by explained variance
        tmp_reducer = KernelPCA(
            n_components=feats.shape[-1],
            gamma=(1 / feats.shape[-1]),
            kernel=params["reduce_dim"]["kernel"],
            random_state=seed,
        )
        if scaler:
        ...
        logger.log.info(
            f"Using {n_components} components to explain {params['reduce_dim']['dim']}"
        )
        reducer = KernelPCA(
        ...
        reducer = KernelPCA(
            n_components=params["reduce_dim"]["dim"],
            gamma=(1 / feats.shape[-1]),
            kernel=params["reduce_dim"]["kernel"],
            random_state=seed,
        )
