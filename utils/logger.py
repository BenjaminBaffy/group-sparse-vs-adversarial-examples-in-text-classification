...
class Logger:
    def __init__(self, log_path):
        ...
        logging.basicConfig(
            filename=f"{log_path}/out.log", level=logging.INFO, filemode="a"
        )
    ...
    def log_metric(self, metric_dict):
        for key, value in metric_dict.items():
            assert (
                key in self.metric_names
            ), f"Trying to log {key}: Not in {self.metric_names} "
    ...
    def save_metric(self):
        ...
        with open(csv_path, "a", newline="") as f:
            ...
            if not exist_flag:
                wr.writerow(headers)
            data = [
                self.metric[self.metric_names[i]] for i in range(len(self.metric_names))
            ]
