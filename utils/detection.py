...
def get_train_features(model_wrapper, args, batch_size, dataset, text_key, layer=-1):
    assert (
        layer == "pooled" or layer < 0
    ), "Layer either has to be a int between -12~-1 or the pooling layer"
...
def get_test_features(model_wrapper, batch_size, dataset, params, logger=None):
    ...
    with torch.no_grad():
        for i in tqdm(range(num_batches)):
            ...
            if len(examples) == 0:
                continue
            output = model_wrapper.inference(
                examples, output_hs=True, output_attention=True
            )
            _, pred = torch.max(output.logits, dim=1)
            feat = output.hidden_states[layer][
                :, 0, :
            ].cpu()  # output.hidden_states : (Batch_size, sequence_length, hidden_dim)
...
def compute_dist(
    ...
    for (mu, cov) in train_stats:
        ...
        log_likelihood = (0.5 * neg_dist) + math.log(
            (2 * math.pi) ** (-mu.shape[0] / 2)
        )
    ...
    confidence, conf_indices = torch.max(
        output, dim=1
    )  # Takes the max of class conditional probability
    if use_marginal:
...
def detect_attack(
    ...
    testset[
        "negative_conf"
    ] = -conf  # negative of confidence : likelihood of adv. probability
