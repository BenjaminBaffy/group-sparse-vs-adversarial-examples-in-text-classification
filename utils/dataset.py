...
def get_dataset(args):
    ...
    if args.dataset == "mnli":
        ...
        text_key = (
            "text" if (args.dataset in ["ag-news", "imdb", "yelp"]) else "sentence"
        )
        testset_key = (
            "test" if (args.dataset in ["ag-news", "imdb", "yelp"]) else "validation"
        )
...
def split_dataset(dataset, split="trainval", split_ratio=0.8):
    ...
    if split == "trainval":
        ...
        train_idx, val_idx = (
            indices[: int(num_samples * split_ratio)],
            indices[int(num_samples * split_ratio) :],
        )
...
def read_testset_from_csv(
    filename,
    use_original=False,
    split_type="random_sample",
    seed=0,
    max_adv_num=500,
    batch_size=64,
    model_wrapper=None,
    ...
    if split_type == "random_sample":
        ...
        attack_success = df.loc[df.result_type == 1][
            ["perturbed_text", "result_type", "ground_truth_output"]
        ]
        ...
        if use_original:
            ...
            text_type = (
                "perturbed_text" if split_type == "attack_scenario" else "original_text"
            )
            attack_failed = df.loc[df.result_type == 0][
                [text_type, "result_type", "ground_truth_output"]
            ]
        ...
        adv_samples = df.loc[df.result_type == 1][
            ["perturbed_text", "result_type", "ground_truth_output"]
        ]
        ...
        clean_samples = df[
            ["original_text", "result_type", "ground_truth_output"]
        ]  # Take all samples (correct and incorrect)
